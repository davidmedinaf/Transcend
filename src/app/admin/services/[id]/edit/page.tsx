    ...
    async function fetchService() {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(json.error?.message || 'Failed to load service');
        }
        const json = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setServerError(json.error?.message || 'Failed to update service');
        return;
      }
    ...
    return (
      <div className="max-w-2xl">
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-white/50 text-sm">Loading service...</div>
        </div>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="max-w-2xl">
        ...
        <div className="rounded-brand border border-red-500/50 bg-red-500/10 p-4">
          <p className="text-sm text-red-400">{loadError}</p>
          <Link
            href="/admin/services"
            className="text-xs text-red-300 underline mt-2 inline-block"
          >
            Return to services
          </Link>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {existingImageUrl && !formData.image && (
            <div className="mb-2">
              ...
              <img
                ...
                alt="Current service"
                className="h-20 w-20 object-cover rounded-brand border border-white/10"
              />
            </div>
          )}
          <div className="flex items-center gap-3">
            ...
            >
              {existingImageUrl ? 'Replace Image' : 'Choose File'}
            </button>
